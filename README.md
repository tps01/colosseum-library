...
Reusable **copy-out** test scripts for Colosseum benches. This is testware, not a
...
2. Replace every `FILL_IN_HERE` in the Python script (units are in trailing comments).
3. Wire instruments:
   - **Autoconfig** (VISA gear): leave `USE_AUTOCONFIG = True`, or call
     `col.equipment.autoconfig(export_path="bench.toml")` once and copy IDs into
     the script.
   - **TOML**: copy `bench.toml.example` → `bench.toml`, set resources, set
     `USE_AUTOCONFIG = False`.
4. Install the plugins listed in the test docstring (same env as `colosseum-core`).
5. Run:

```sh
colosseum run path/to/test_rail_voltage.py
# or with an explicit config when USE_AUTOCONFIG is False:
colosseum run path/to/test_rail_voltage.py --config path/to/bench.toml
```
...
| `docs/target_apis.md` | TDD contracts referenced by catalog scripts |
...
|-------|--------------|-------------|
| Instruments | VISA resources, models, IDs | `col.equipment.autoconfig()` and/or `bench.toml` |
| Procedure | Frequencies, limits, pins, DUT IPs | `FILL_IN_HERE` in the Python file |

Autoconfig today discovers VISA instruments only. DIO, serial, SSH, host profile,
...
Paths in suite TOML are relative to the suite file. See [suites/README.md](suites/README.md).
...
4. Keeps `FILL_IN_HERE` in the committed catalog; wiring goes in `# SETUP:` comments.
...
scripts call TDD APIs not yet implemented in plugins; equipment rises to meet them.

- **digital** — `colosseum-equipment`, `colosseum-shared`, `colosseum-host`, `colosseum-messaging`
